...
DSH Remote 通过你的私有 Tailscale 网络，为 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 提供专为手机设计的工作界面。离开电脑也能发起任务、实时跟进 Agent、处理审批并检查结果，同时不把 Harness 暴露到公网。
...
| `packages/remote-domain` | Host、工作区、任务、审批、问题和回顾模型 |
